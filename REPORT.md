...
- **탐색한 부분그래프 시각화**: 답변 아래 "그래프로 보기"를 펼치면 이번 질문에서 실제로 방문한 노드·엣지만(전체 121개가 아니라) `pyvis`로 인터랙티브하게 보여준다. 노드 타입별로 색을 다르게 하고 시작 개체는 더 크게 그려서, 텍스트로 된 트리플 목록보다 한눈에 "어디서 어디로 갔는지"를 알 수 있게 했다.

![데모 화면](assets/demo_screenshot.png)

![탐색한 부분그래프](assets/demo_graph_view.png)
