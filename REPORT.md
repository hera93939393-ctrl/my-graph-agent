...
- **생성 실패 1건 (Q6)**: "가득축산과 같은 위반사항(냉장온도_기준초과)을 받은 다른 업체는 어떤 제재를 받았는가?" — 근거 재현율은 1.0(든든축산·온누리키친·청솔키친 문서 전부 확보)이었는데도, LLM이 "이들 업체는 모두 이용제한 1개월의 제재를 받았습니다"라고 답했다. 실제로는 든든축산만 이용제한_1개월이고 온누리키친·청솔키친은 서류_보완요청이다 — 근거에 세 업체가 서로 다른 제재를 받았다고 분명히 적혀 있는데도 LLM이 하나로 뭉뚱그린 **진짜 환각**이다. 검색은 100% 성공했고 생성 단계에서만 깨진, 이 분류 체계가 정확히 잡아내야 하는 유형의 실패다.

  (LLM 연결 전에는 다른 문항(Q3, "가온식품이 받은 제재조치는?")이 생성 실패로 잡혔었다 — 근거는 맞게 모았는데 그때는 `OPENAI_API_KEY` 없이 규칙 기반 요약만 돌려서 "해당없음"이라는 결론 문장을 스스로 못 만들었기 때문. LLM을 연결하자 Q3는 정상적으로 "정보가 없다"고 답해 해결됐고, 대신 Q6에서 진짜 환각이 드러났다 — 근거 재현율이 1.0이어도 생성 단계 검증이 왜 별도로 필요한지 보여주는 사례다.)
...
위는 "가득축산과 같은 지역본부에 등록된 다른 업체는 어디인가?"를 실제로 물어본 결과다. 사용 홉 수(2)·근거 문서 수(5건) 배지, 허브 노드 상한(5건) 적용 캡션("경북지역본부")까지 화면에서 확인할 수 있다.
...
2. LLM을 실제로 연결해보니(3절), 업체마다 다른 값을 가진 관계(예: 위반유형별 제재)를 물으면 LLM이 여러 업체의 사실을 하나로 뭉뚱그려 답하는 환각이 나왔다 — 근거 재현율이 1.0이어도 생성 단계에서 깨질 수 있다는 걸 실측으로 확인했다. 시스템 프롬프트에 "업체별로 구분해서 답하라"는 지시를 추가하거나, 답변 생성 후 각 문장이 근거 삼중항과 1:1로 대응하는지 자동 검증하는 후처리 단계를 추가하면 줄일 수 있을 것이다.
