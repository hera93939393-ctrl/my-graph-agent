...
from graph_viz import filter_nodes_by_answer, render_subgraph_html
...
for result in st.session_state.history:
    ...
    if result.get("visited_nodes"):
        relevant_nodes = filter_nodes_by_answer(g, result["visited_nodes"], result["start_nodes"], result["answer"])
        with st.expander(f"그래프로 보기 ({len(relevant_nodes)}개 노드 — 답변에 나온 것만)", expanded=False):
            html = render_subgraph_html(g, relevant_nodes, result["start_nodes"])
